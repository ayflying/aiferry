...
function openEdit(item: APIKey) {
  editing.value = item
  created.value = undefined
  Object.assign(form, { name: item.name, status: item.status, expiresAt: item.expiresAt ? new Date(item.expiresAt) : undefined, spendLimit: item.spendLimit ?? undefined, dailySpendLimit: item.dailySpendLimit ?? undefined, allowedModels: [...(item.allowedModels ?? [])], channelGroupIds: [...(item.channelGroupIds ?? [])] })
  dialogOpen.value = true
}
