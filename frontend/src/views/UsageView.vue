  ...
  <div class="page-stack">
    ...
    <div class="table-panel">
      <el-table v-loading="loading" :data="usageItems" row-key="id">
        ...
        <el-table-column label="状态" width="86"><template #default="{ row }"><el-tag :type="row.httpStatus >= 200 && row.httpStatus < 300 ? 'success' : 'danger'" effect="plain" size="small">{{ row.httpStatus }}</el-tag></template></el-table-column>
        ...
        <el-table-column label="估算成本" min-width="125"><template #default="{ row }"><span :class="row.estimatedCost === undefined ? 'muted' : 'mono'">{{ formatCost(row.estimatedCost) }}</span></template></el-table-column>
...
<style scoped>
.request-cell, .token-cell { display: flex; min-width: 0; flex-direction: column; gap: 3px; }.request-cell > span, .request-cell > strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.request-cell small, .token-cell small { color: #7b8792; font-size: 10px; }.token-cell strong { font-family: 'JetBrains Mono', monospace; font-size: 11px; }
</style>
