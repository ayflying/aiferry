...
import { currentTimeInDisplayZone, formatCost, formatNumber, formatPreciseCost, formatReasoningEffort, formatTokenSpeed, formatTime, formatUsageDuration } from '../lib/format'
...
// 模型健康评分分色：与渠道模型测试弹窗一致（≥80 绿、≥50 黄、<50 红）
function modelHealthTone(score: number) {
  if (score >= 80) return 'latency-fast'
  if (score >= 50) return 'latency-medium'
  return 'latency-slow'
}
  ...
  <div class="page-stack">
    ...
    <div class="table-panel">
      <ResponsiveList>
        ...
        <el-table-column :label="isAdmin ? '渠道 / 密钥' : '访问密钥'" min-width="190"><template #default="{ row }"><ChannelCredentialDisplay v-if="isAdmin" class="request-cell" :channel-name="row.channelName" :credential-index="row.channelCredentialIndex" :api-key-name="row.apiKeyName" /><div v-else class="request-cell"><span>{{ row.apiKeyName || '—' }}</span></div></template></el-table-column>
        <el-table-column label="状态" min-width="110"><template #default="{ row }"><div class="status-cell"><el-tag :type="isSuccessful(row) ? 'success' : 'danger'" effect="plain" size="small">{{ row.httpStatus }}</el-tag><small v-if="row.modelHealthScore != null" :class="modelHealthTone(row.modelHealthScore)">健康分 {{ row.modelHealthScore }}</small><small v-else class="latency-unknown">健康分 —</small></div></template></el-table-column>
        <el-table-column label="流式" min-width="96"><template #default="{ row }"><div class="stream-cell"><strong>{{ streamLabel(row) }}</strong><small>{{ formatTokenSpeed(row.outputTokens, row.durationMs) }}</small></div></template></el-table-column>
        ...
        <el-table-column label="耗时" min-width="116"><template #default="{ row }"><div class="latency-cell"><span class="latency-strip" :class="{ 'single-latency': !row.isStream }"><i :class="row.isStream ? firstTokenTone(row) : totalLatencyTone(row)" /><i v-if="row.isStream" :class="totalLatencyTone(row)" /></span><div class="latency-copy"><template v-if="row.isStream"><strong>首字 <span :class="firstTokenTone(row)">{{ formatUsageDuration(row.firstTokenMs) }}</span></strong><small>耗时 <span :class="totalLatencyTone(row)">{{ formatUsageDuration(row.durationMs) }}</span></small></template><template v-else><strong>响应 <span :class="totalLatencyTone(row)">{{ formatUsageDuration(row.durationMs) }}</span></strong><small>非流式响应</small></template></div></div></template></el-table-column>
        ...
        <template #mobile><MobileRecordList :loading="loading">
          <article v-for="row in usageItems" :key="row.id" class="mobile-record">
            <div class="mobile-record__header"><div class="mobile-record__title"><strong>{{ row.requestedModel }}</strong><small>{{ formatTime(row.createdAt) }} · {{ formatIPLocation(row.ipLocation) }}</small></div><div class="mobile-record__status"><el-tag :type="isSuccessful(row) ? 'success' : 'danger'" effect="plain" size="small">{{ row.httpStatus }}</el-tag><small v-if="row.modelHealthScore != null" :class="modelHealthTone(row.modelHealthScore)">健康分 {{ row.modelHealthScore }}</small><small v-else class="latency-unknown">健康分 —</small></div></div>
            ...
            <div class="mobile-record__footer"><span class="muted">{{ streamLabel(row, true) }}</span><el-button size="small" @click="openUsageDetail(row)">{{ isSuccessful(row) ? '查看计费详情' : '查看失败日志' }}</el-button></div>
          </article>
...
.status-cell { display: flex; flex-direction: column; align-items: center; gap: 3px; }.status-cell small { font-size: 11px; line-height: 1.2; }
.mobile-record__status { display: flex; flex-direction: column; align-items: flex-end; gap: 3px; white-space: nowrap; }.mobile-record__status small { font-size: 11px; line-height: 1.2; }
</style>
