      ...
      <el-tab-pane name="types">
        ...
        <div class="table-panel">
          <el-table v-loading="loading" :data="store.channelTypes" row-key="id">
            ...
            <el-table-column label="状态" width="142"><template #default="{ row }"><div class="type-status-control"><span class="status-dot" :class="row.status === 1 ? 'success' : ''">{{ row.status === 1 ? '启用' : '停用' }}</span><el-switch :model-value="row.status === 1" :disabled="typeStatusSaving[row.id]" :aria-label="`${row.name} ${row.status === 1 ? '已启用' : '已停用'}`" @update:model-value="setTypeStatus(row, $event)" /></div></template></el-table-column>
