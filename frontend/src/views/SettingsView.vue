...
<template>
  <div v-loading="activeTabLoading" class="page-stack settings-page">
    ...
    <template v-else-if="activeTab === 'resilience'">
      ...
      <section class="settings-section"><div class="section-heading"><div><h2>后台渠道探测</h2><span>使用已启用模型执行最小请求，不保存提示词或响应正文。</span></div><el-switch v-model="form.healthCheckEnabled" /></div><el-form label-position="top" class="settings-form"><el-form-item label="检查间隔（分钟）"><el-input-number v-model="form.healthCheckIntervalMinutes" :disabled="!form.healthCheckEnabled" :min="1" :max="1440" controls-position="right" /></el-form-item></el-form><div class="setting-switch probe-mode"><div><strong>仅被动恢复</strong><span>只检查真实中转请求失败后自动禁用的渠道是否可恢复；关闭后会检查全部可管理渠道。</span></div><el-switch :model-value="form.healthCheckMode === 'passive'" :disabled="!form.healthCheckEnabled" @update:model-value="form.healthCheckMode = $event ? 'passive' : 'all'" /></div></section>
