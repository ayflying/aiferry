...
<template>
  <div v-loading="loading" class="page-stack settings-page">
    ...
    <template v-else>
      <section class="settings-section"><div class="section-heading"><div><h2>余额提醒</h2><span>用户余额低于阈值时，每 24 小时最多发送一次提醒。</span></div><el-switch v-model="mailForm.enabled" /></div><el-form label-position="top" class="settings-form"><el-form-item label="提醒阈值（美元）"><el-input-number v-model="mailForm.threshold" :min="0" :max="1000000" :precision="6" controls-position="right" /></el-form-item></el-form></section>
