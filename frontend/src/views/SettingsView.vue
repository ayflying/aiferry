...
<template>
  <div v-loading="activeTabLoading" class="page-stack settings-page">
    ...
    <template v-else-if="activeTab === 'basic'">
      ...
      <section class="settings-section"><div class="section-heading"><div><h2>公开首页</h2><span>控制根地址是否展示公开首页。</span></div></div><div class="setting-switch"><div><strong>允许访问公开首页</strong><span>关闭后，访问根地址会跳转至登录页；默认关闭。</span></div><el-switch v-model="informationForm.publicHomepageEnabled" /></div></section>
