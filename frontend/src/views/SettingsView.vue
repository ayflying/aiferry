...
async function loadBasic() {
  ...
  try {
    const [basicSettings, informationSettings] = await Promise.all([
      apiGet<BaseSettings>('/system/basic'),
      apiGet<SystemInformationSettings>('/system/information'),
    ])
...
<template>
  <div v-loading="activeTabLoading" class="page-stack settings-page">
    ...
    <template v-else>
      <section class="settings-section"><div class="balance-reminder-row"><div class="balance-reminder-title"><h2>用户余额提醒</h2><span>用户在 24 小时内有模型使用且余额低于阈值时，最多发送一次提醒；没有使用不发送。</span></div><label class="balance-reminder-threshold"><span>提醒阈值（美元）</span><el-input-number v-model="mailForm.threshold" :min="0" :max="1000000" :precision="6" controls-position="right" aria-label="提醒阈值（美元）" /></label><el-switch v-model="mailForm.enabled" /></div><el-form label-position="top" class="settings-form"><div class="form-subsection"><strong>提醒模板</strong><span>可使用 {nickname}、{balance}、{threshold}、{system_name} 和 {url} 作为变量。</span></div><el-form-item label="邮件主题"><el-input v-model="mailForm.subjectTemplate" /></el-form-item><el-form-item label="邮件正文"><el-input v-model="mailForm.bodyTemplate" type="textarea" :rows="8" spellcheck="false" /></el-form-item></el-form></section>
...
.settings-section:has(.smtp-test) .settings-form { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); column-gap: 14px; }
