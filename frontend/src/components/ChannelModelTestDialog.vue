...
function healthScoreClass(model: ChannelModel) {
  if (model.autoDisabled) return 'danger'
...
<template>
  <el-dialog v-model="visible" :title="`测试渠道连接：${channel?.name || ''}`" width="min(880px, calc(100vw - 16px))" class="channel-test-dialog" destroy-on-close>
...
@media (max-width: 600px) {
  ...
  :deep(.channel-test-dialog .el-dialog__header) { padding: 16px 16px 12px; }
  :deep(.channel-test-dialog .el-dialog__body) {
    max-height: calc(100vh - 168px);
    overflow-y: auto;
    padding: 0 16px 16px;
  }
  :deep(.channel-test-dialog .el-dialog__footer) {
    display: flex;
    gap: 8px;
    padding: 12px 16px 16px;
  }
  :deep(.channel-test-dialog .el-dialog__footer .el-button) {
    flex: 1;
    margin-left: 0;
  }
  .test-settings { grid-template-columns: 1fr; gap: 14px; padding-bottom: 14px; }
  .test-section-heading { align-items: stretch; flex-direction: column; gap: 12px; }
  .model-filter { width: 100%; }
  .test-actions { align-items: stretch; flex-direction: column; }
  .test-actions .el-button, .result-chip { width: 100%; justify-content: center; }
  .test-pagination { flex-wrap: wrap; justify-content: space-between; gap: 8px; }
  .test-pagination :deep(.el-pagination) { width: 100%; justify-content: center; }
  .test-table-wrap { overflow-x: auto; }
  .test-model-table { min-width: 660px; }
}
</style>
