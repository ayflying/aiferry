...
import { CircleAlert, CircleCheck, Gauge, Info, Trash2 } from '@lucide/vue'
  ...
  <el-dialog v-model="visible" :title="`测试渠道连接：${channel?.name || ''}`" class="channel-test-dialog" destroy-on-close>
    ...
    <section class="model-test-section">
      ...
      <div class="test-actions">
        <el-button type="primary" :loading="running" :disabled="!enabledModels.length" @click="testAll">测试全部 {{ enabledModels.length }} 个模型</el-button>
        <span class="result-chip success"><CircleCheck :size="15" /> 测试通过（{{ successCount }}）</span>
        <el-button v-if="failedModels.length" plain type="danger" :icon="Trash2" :disabled="running" @click="deleteFailedModels">删除失败模型（{{ failedModels.length }}）</el-button>
      </div>
      ...
      <div v-loading="loading" class="test-table-wrap">
        <el-table v-if="pagedModels.length" :data="pagedModels" :show-header="false" size="small" class="test-model-table" height="360">
          <el-table-column width="44" align="center"><template #default="{ row }"><el-radio v-model="selectedModelID" :value="row.id" :aria-label="`选择 ${row.publicName}`" /></template></el-table-column>
          ...
          <el-table-column width="104"><template #default="{ row }"><span v-if="statusOf(row) === 'success'" class="test-status success">成功</span><span v-else-if="statusOf(row) === 'failed'" class="test-status failed">失败</span><span v-else class="test-status idle">未测试</span></template></el-table-column>
          ...
          <el-table-column width="54" align="center"><template #default="{ row }"><el-tooltip content="测试此模型"><button class="icon-button" type="button" :aria-label="`测试 ${row.publicName}`" :disabled="running" @click="runTest(row)"><Gauge :size="17" /></button></el-tooltip></template></el-table-column>
        </el-table>
    ...
    <template #footer><el-button @click="visible = false">关闭</el-button><el-button type="primary" :disabled="!selectedModelID || running" @click="testSelected">测试选中模型</el-button></template>
  </el-dialog>
</template>

<style scoped>
:deep(.channel-test-dialog) { width: min(880px, calc(100vw - 32px)) !important; }.test-settings { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 20px; padding-bottom: 18px; border-bottom: 1px solid #dce2e7; }.setting-field { display: grid; gap: 8px; }.setting-field label, .test-section-heading h3 { color: #15202b; font-size: 13px; font-weight: 600; }.setting-field small, .test-section-heading p, .model-name small, .test-outcome small { color: #7b8792; font-size: 11px; }.model-test-section { padding-top: 18px; }.test-section-heading { display: flex; align-items: end; justify-content: space-between; gap: 18px; }.test-section-heading h3, .test-section-heading p { margin: 0; }.test-section-heading p { margin-top: 6px; }.model-filter { width: 258px; }.test-actions, .test-pagination { display: flex; align-items: center; gap: 9px; }.test-actions { min-height: 36px; margin: 12px 0; flex-wrap: wrap; }.result-chip, .test-status { display: inline-flex; align-items: center; gap: 4px; font-size: 12px; font-weight: 600; }.result-chip { min-height: 30px; padding: 0 10px; border: 1px solid #dce2e7; border-radius: 15px; font-weight: 500; }.result-chip.success, .test-status.success { color: #16866f; }.test-status.failed { color: #c83e4d; }.test-status.idle, .muted { color: #7b8792; }.test-table-wrap { overflow: hidden; border: 1px solid #dce2e7; border-radius: 8px; background: #fff; }.test-model-table :deep(.el-table__cell) { height: 52px; }.model-name, .test-outcome { display: flex; min-width: 0; flex-direction: column; gap: 3px; }.model-name strong, .test-outcome > span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.model-name strong { font-family: 'JetBrains Mono', monospace; font-size: 12px; }.test-outcome.failed { display: flex; flex-direction: row; align-items: center; color: #66717d; }.test-outcome.failed > span { flex: 1; }.detail-button { display: inline-flex; align-items: center; gap: 4px; border: 0; color: #40505f; background: transparent; cursor: pointer; font: inherit; font-size: 11px; }.detail-button:hover { color: #1677ff; }.error-detail { margin: 0; color: #40505f; overflow-wrap: anywhere; line-height: 1.55; }.test-empty { display: flex; min-height: 210px; align-items: center; justify-content: center; gap: 8px; color: #7b8792; font-size: 12px; }.test-pagination { justify-content: end; min-height: 48px; color: #7b8792; font-size: 11px; }.page-size { width: 68px; }@media (max-width: 680px) { .test-settings { grid-template-columns: 1fr; gap: 14px; }.test-section-heading { align-items: stretch; flex-direction: column; gap: 12px; }.model-filter { width: 100%; }.test-table-wrap { overflow-x: auto; }.test-model-table { min-width: 700px; }.test-pagination { justify-content: flex-start; flex-wrap: wrap; } }
</style>
