...
<template>
  ...
  <section class="settings-section quality-history-section">
    ...
    <el-table v-loading="loading" :data="events.items" row-key="id" empty-text="暂无检测历史">
      ...
      <el-table-column label="渠道 / 密钥" min-width="190"><template #default="{ row }"><ChannelCredentialDisplay class="cell-stack" :channel-name="row.channelName" :credential-index="row.credentialIndex" :api-key-name="row.apiKeyName" /></template></el-table-column>
