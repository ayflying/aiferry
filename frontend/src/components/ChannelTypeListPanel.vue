...
import { channelTypeCostLabel, channelTypeQueryLabel } from '../lib/channelTypeDisplay'
    ...
    <ResponsiveList>
      <template #desktop>
        <el-table v-loading="props.loading" :data="props.types" row-key="id">
          ...
          <el-table-column label="查询能力" min-width="180"><template #default="{ row }"><span>{{ channelTypeQueryLabel(row) }}</span><small v-if="row.config.costs.path"> · {{ row.config.costs.path }}</small></template></el-table-column>
      ...
      <template #mobile>
        <MobileRecordList :loading="props.loading">
          <article v-for="row in props.types" :key="row.id" class="mobile-record">
            ...
            <dl class="mobile-record__facts"><div class="mobile-record__wide"><dt>模型接口</dt><dd class="mono">{{ row.config.models.method }} {{ row.config.models.path }}</dd></div><div><dt>查询能力</dt><dd>{{ channelTypeQueryLabel(row) }}<span v-if="row.config.costs.path"> · {{ row.config.costs.path }}</span></dd></div><div><dt>来源</dt><dd>{{ row.builtIn === 1 ? '内置' : '自定义' }}</dd></div></dl>
