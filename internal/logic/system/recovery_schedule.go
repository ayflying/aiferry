...
func (s *sSystem) recordAutoDisableFailure(ctx context.Context, channelID, credentialID uint64, threshold int) (bool, error) {
	if threshold < 1 {
...
func (s *sSystem) recordConsecutiveFailure(ctx context.Context, counterKey, cooldown string, threshold int) (bool, error) {
...
func (s *sSystem) ClearAutoDisableFailures(ctx context.Context, credentialID uint64) {
	if credentialID == 0 {
		return
	}
...
func (s *sSystem) ClearChannelAutoDisableFailures(ctx context.Context, channelID uint64) {
	if channelID == 0 {
		return
	}
...
func (s *sSystem) ResetCredentialRecoverySchedule(ctx context.Context, credentialID uint64) {
	s.clearRecoverySchedule(ctx, RecoveryTargetCredential, credentialID)
}

func (s *sSystem) ResetChannelRecoverySchedule(ctx context.Context, channelID uint64) {
	s.clearRecoverySchedule(ctx, RecoveryTargetChannel, channelID)
}

func (s *sSystem) BeginRecoveryAttempt(ctx context.Context, target RecoveryTarget, id uint64, autoDisabledAt time.Time) (bool, error) {
...
func (s *sSystem) FinishRecoveryAttempt(ctx context.Context, target RecoveryTarget, id uint64, succeeded bool) {
	if succeeded {
...
func (s *sSystem) resetRecoverySchedule(ctx context.Context, target RecoveryTarget, id uint64) {
	s.clearRecoverySchedule(ctx, target, id)
}

func (s *sSystem) clearRecoverySchedule(ctx context.Context, target RecoveryTarget, id uint64) {
	if id == 0 {
		return
	}
...
func (s *sSystem) loadRecoveryState(ctx context.Context, target RecoveryTarget, id uint64) (recoveryState, error) {
