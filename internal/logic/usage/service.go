...
type LogView struct {
	...
	ChannelCredentialPrefix string            `json:"channelCredentialPrefix" orm:"channel_credential_prefix"`
	ChannelCredentialIndex  uint              `json:"channelCredentialIndex" orm:"channel_credential_index"`
...
func New(location IPLocator, settings ...*system.Service) *sUsage {
	service := &sUsage{location: location}
	if len(settings) > 0 {
...
func (s *sUsage) timeLocation(ctx context.Context) *time.Location {
	if s.settings == nil {
		return time.Local
	}
...
func (s *sUsage) Record(ctx context.Context, input RecordInput) error {
