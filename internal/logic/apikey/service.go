...
func New(appSvc *app.Service) *sAPIKey {
	return &sAPIKey{app: appSvc}
}

func (s *sAPIKey) List(ctx context.Context) ([]View, error) {
...
func (s *sAPIKey) Create(ctx context.Context, input adminapi.APIKeyInput) (Created, error) {
...
func (s *sAPIKey) Reveal(ctx context.Context, id uint64) (string, error) {
...
func (s *sAPIKey) Update(ctx context.Context, id uint64, input adminapi.APIKeyUpdate) error {
	var current AuthKey
...
func (s *sAPIKey) Delete(ctx context.Context, id uint64) error {
	var current AuthKey
	if err := dao.ApiKeys.Ctx(ctx).Where(dao.ApiKeys.Columns().Id, id).Scan(&current); err != nil {
		return gerror.Wrap(err, "find API key")
	}
...
func (s *sAPIKey) ensureAccess(ctx context.Context, ownerID uint64) error {
	current, ok := auth.CurrentUser(ctx)
	if !ok {
