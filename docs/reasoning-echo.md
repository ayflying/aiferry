...
- 存档依赖 Redis 与同一个 API 密钥。**Redis 重启、键超过 24 小时、或客户端切换了 API 密钥时
  ...
  上下文。注意空串兜底对「有状态会话」上游（见下节 ch9）无效，那类渠道会据此返回 400；
  0.5.124 起这类 400 不再直接抛给客户端，而是继续路由到下一个候选。
...
### 修复：这类 400 不再是对客户端的最终判决（0.5.124）

上面两条推论直接决定了修法——**既然换渠道就能自愈，就不能把它当成「客户端请求错」直接返回**。
改动落在两处：

1. `internal/logic/relay/credential_retry.go` 的 `nonRetryableClientFailure` 新增
   `upstreamStatefulSessionRejection(status, body, errorMessage)`：**状态码是 400/422 且响应里点名
   `reasoning_content`** 时返回 false，把该候选交还候选循环继续路由。认定依据是「上游把失败归因
   于会话状态」这一事实，所以不依赖管理端 `RetryStatusCodes` 是否包含 400（默认配置不含 400，
   通用场景行为不变）；判定只看字段名、不绑定整句措辞，宁可多换一次渠道也不漏判。
2. `internal/logic/relay/service.go` 的候选耗尽收尾：原来只在「状态码不在 `RetryStatusCodes` 里」
   时才透传上游响应，于是**所有候选都以 400 失败**会被改写成 503 `server_error`（客户端据此重试，
   与事实不符）。现在只要最后是一次**尚未写出内容的 4xx** 就原样透传，「某状态码是否可重试」只
   决定要不要继续换候选，不再改变最终对客户端的表达。

行为对照（同一 key、同一份请求）：

| 场景 | 修复前 | 修复后 |
| --- | --- | --- |
| 第一个候选是 ch9（会话状态 400），后面还有可用候选 | 立即把 400 返回客户端，`attempts=1` | 继续试下一个候选，落到无状态渠道即 200 |
| 所有候选都以这类 400 失败 | 400 | 400（透传最后一个上游响应，语义不变） |
| 真正的客户端请求错（如 `messages` 缺字段）撞上 400 | 400 | 400（判定不命中，仍是最终判决） |

因此**不需要停用或降权 ch9**：它的额度继续可用，偶发拒绝会自动落到能受理的候选上。
